...
public class Model {
    ...
    private final ArrayList<HeroShell> listShell = new ArrayList<>();
    ...
    public void moveHero(Permeshalka p){
        ...
        for (GameObjects obj:listObjects){
            if (!obj.equals(heroes)) {
                switch (p) {
                    case UP:
                        if (obj.collision(heroes)||heroes.getY()<= heroes.getHight() / 2) {
                    ...
                    case LEFT:
                        if (obj.collision(heroes)||heroes.getX()<= heroes.getWeight() / 2) {
